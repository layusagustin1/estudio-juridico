              ...
              <div className="border-t border-primary">
                 {practiceAreas.map(({ number, title, description, localNote, icon: Icon }) => (
                  <article key={number} data-testid={`card-practice-${number}`} className="group relative grid gap-4 border border-transparent border-b-border py-7 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-accent/70 hover:bg-secondary/70 hover:shadow-[0_14px_30px_hsl(var(--primary)/.1)] sm:grid-cols-[60px_1fr_44px] sm:items-center sm:gap-7">
                    ...
                    <span className="hidden h-10 w-10 items-center justify-center border border-border text-primary transition-colors group-hover:border-accent group-hover:bg-accent group-hover:text-accent-foreground sm:flex"><ArrowUpRight size={16} aria-hidden="true" /></span>
                  </article>
                ))}
              </div>
            </div>
